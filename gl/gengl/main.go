...
// #cgo LDFLAGS: -lstdc++
{{if eq $.GLVersionName "ES2"}}// #cgo linux freebsd LDFLAGS: -lGL
// #cgo windows LDFLAGS: -lopengl32
// #cgo darwin LDFLAGS: -framework OpenGL
{{end}}// #cgo pkg-config: Qt6Core Qt6OpenGL
//
// #include "funcs.h"
//
// void free(void*);
...
{{if eq $.Class "QOpenGLFunctions"}}#include <QtGui/{{lower $.Class}}.h>
{{else}}#include <QOpenGLVersionFunctionsFactory>
#include <QtOpenGL/{{lower $.Class}}.h>
{{end}}
#include "funcs.h"

void *gl{{$.GLVersionLabel}}_funcs() {
	{{$.Class}}* funcs = {{if eq $.Class "QOpenGLFunctions"}}QOpenGLContext::currentContext()->functions(){{else}}QOpenGLVersionFunctionsFactory::get<{{$.Class}}>(QOpenGLContext::currentContext()){{end}};
	if (!funcs) {
		return 0;
